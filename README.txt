...
Книга 04 (хаб) — main_04.gs, sync_hub.gs, gremlin_scheduled.gs, sender_stock.gs, costing.gs.
  sender_stock.gs и costing.gs нужны для цепочки «04→02→05» (меню Операционные потоки); те же файлы заливайте и в книги 02/05 при обновлении.
  Меню «Синхронизация книг → Операционные потоки → Цепочка отгрузки 04→02→05» (dry-run / боевой).
  Расписание: «⏱ Расписание → Установить триггеры этой книги (04)».

- costing_receipt_3pl.gs: книга 05; приёмка на складе РФ. Меню «📥 Приёмка (3PL)»: загрузка CSV/XLSX от 3PL, сверка с «Партии_в_рейсе» и «Декларации_строки» → лист «Расхождения_приемки» (SHORTAGE/SURPLUS/DEFECT/ASSORTMENT/CUSTOMS_VS_PHYSICAL). «Применить подтверждённые к партиям» — только строки со статусом «подтверждено», автоснимок партий, обновление «Количество» и «Qty_для_аллокации» (годные шт; пересчёт себестоимости использует эту колонку). Листы: «Приемка_журнал», «Приемка_строки», «Расхождения_приемки», «Справочник_маппинг_3PL». Свойство RECEIPT_CUSTOMS_QTY_TOLERANCE — допуск шт при сверке ДТ vs физика (по умолчанию 0). Событие RECEIPT_ACCEPTED в «События_рейса» — пункт меню или шаблон в «Типы_событий» (logistics.gs). В «Сводную» (01) приёмка не пишется.

Книга 05 (себестоимость + оплаты логистики) — состав файлов для деплоя в её GAS-проект:
- main_05.gs (один onOpen, который подключает меню), costing.gs, costing_customs_xml.gs, costing_receipt_3pl.gs, logistics.gs, payment_requests_05.gs, gremlin_scheduled.gs.
